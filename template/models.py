...
class CNN(chainer.Chain):
    ...
    def __call__(self, x):
        h = F.max_pooling_2d(F.relu(self.conv1(x)), ksize=2, stride=1)
        h = F.max_pooling_2d(F.relu(self.conv2(h)), ksize=2, stride=1)
        h = F.max_pooling_2d(F.relu(self.conv3(h)), ksize=2, stride=1)
        h = F.relu(self.fc1(h))
        h = F.relu(self.fc2(h))
        return self.fc3(h)
